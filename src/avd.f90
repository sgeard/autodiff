! Created by on 28/02/2020 by Simon Geard
...
module AVD
    ...
    interface operator(**)
        module procedure :: d0_pow_i, d0_pow_r, i_pow_d0, r_pow_d0
        module procedure :: d1_pow_i, d1_pow_r, i_pow_d1, r_pow_d1
        module procedure :: d2_pow_i, d2_pow_r, i_pow_d2, r_pow_d2
    ...
    interface operator( / )
        ...
        pure module function d1_over_d1(obj, a) result(r)
            type(avd_d1) :: r
            type(avd_d1), intent(in) :: obj
            type(avd_d1), intent(in) :: a
        end function d1_over_d1
...
contains
    ...
    ! D2(f/g) = (f''g - 2f'g' - g''f)/g**2 + 2fg'**2/g**3
    pure function d2_over_d2(a, obj) result(r)
        type(avd_d2) :: r
        type(avd_d2), intent(in) :: obj
        type(avd_d2), intent(in) :: a
        r%avd_d1 = a%avd_d1 / obj%avd_d1
        associate(f0=>a%v,   f1=>a%d1,   f2=>a%d2,  &
                  g0=>obj%v, g1=>obj%d1, g2=>obj%d2 )
            r%d2 = (f2*g0 - 2*f1*g1 - g2*f0)/g0**2 + 2*f0*g1**2/g0**3
        end associate
    end function d2_over_d2
    ...
    pure function i_pow_d0(n, obj) result(r)
        type(avd_b) :: r
        type(avd_b), intent(in) :: obj
        integer, intent(in)     :: n
        r%v = n**obj%v
    end function i_pow_d0

    pure function r_pow_d0(x, obj) result(r)
        type(avd_b) :: r
        type(avd_b), intent(in) :: obj
        real(8), intent(in)     :: x
        r%v = x**obj%v
    end function r_pow_d0
    ...
    pure function i_pow_d1(n, obj) result(r)
        type(avd_d1) :: r
        type(avd_d1), intent(in) :: obj
        integer, intent(in)     :: n
        real(8) :: f1
        r%v = n**obj%v
        f1 = log(real(n,8))*n**obj%v
        associate(g1=>obj%d1)
            r%d1 = f1*g1
        end associate
    end function i_pow_d1

    pure function r_pow_d1(x, obj) result(r)
        type(avd_d1) :: r
        type(avd_d1), intent(in) :: obj
        real(8), intent(in)     :: x
        real(8) :: f1
    ...
    end function r_pow_d1

    pure function d2_pow_i(obj, n) result(r)
        type(avd_d2) :: r
        type(avd_d2), intent(in) :: obj
        integer, intent(in)     :: n
        r = d2_pow_r(obj,real(n,8))
    ...
    pure function i_pow_d2(n, obj) result(r)
        type(avd_d2) :: r
        type(avd_d2), intent(in) :: obj
        integer, intent(in)     :: n
        r = r_pow_d2(real(n,8),obj)
    end function i_pow_d2

    pure function r_pow_d2(x, obj) result(r)
        type(avd_d2) :: r
        type(avd_d2), intent(in) :: obj
        real(8), intent(in)     :: x
        real(8) :: f1, f2
        r%avd_d1 = x**obj%avd_d1
        f1 = log(x)*x**obj%v
        f2 = log(x)**2*x**obj%v
        associate(g1=>obj%d1, g2=>obj%d2)
            r%d2 = f2*g1**2 + f1*g2
        end associate
    end function r_pow_d2
